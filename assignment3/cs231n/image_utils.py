...
def set_socks5_proxy(host='localhost', port=9909):
    socks.set_default_proxy(socks.SOCKS5, host, port)
    socket.socket = socks.socksocket
    try:
        print(urllib.request.urlopen('http://cip.cc/', timeout=24).read())
    except:
        pass
    try:
        print(urllib.request.urlopen('http://icanhazip.com/', timeout=24).read())
    except:
        pass
