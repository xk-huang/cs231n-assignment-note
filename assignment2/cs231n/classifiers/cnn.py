...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params["W2"] = np.random.randn(int(np.prod(
            [num_filters, *input_dim[1:]]) / 4), hidden_dim) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        scores, conv_relu_pool_cache = conv_relu_pool_forward(
            X, W1, b1, conv_param, pool_param)
        conv_shape = scores.shape
        ...
        for i in range(1, 4):
            loss += self.reg * np.sum(self.params['W%d' % (i)] ** 2) / 2
        ...
        dout, dW3, db3 = affine_backward(dloss, affine_cache)
